...
def open_csv_writer(name, delimiter='\t', fieldnames=None):
    return csv.DictWriter(gzip.open('{}'.format(name), 'wt'), delimiter=delimiter, fieldnames=fieldnames, escapechar='\\')

def open_csv_reader(name, delimiter='\t', fieldnames=None):
    return csv.DictReader(gzip.open('{}'.format(name), 'rt'), delimiter=delimiter, fieldnames=fieldnames, escapechar='\\')
